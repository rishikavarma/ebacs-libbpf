...
# File paths
input_file = "results_workload_ran.txt"  # Replace with your actual file path
output_csv = "parsed_latencies_ran.csv"
...
for bg in backgrounds:
    ...
    plt.savefig(f"latency_percentiles_bg_uni_{bg}.png")
...
plt.title("Allocstalls for different Memory pressure (Uniform)")
plt.xlabel("Watermark Scale Factor")
plt.ylabel("Allocstall Events")
plt.legend()
plt.grid(True)
plt.xticks(scale_factors)  # Ensure all scale factors are shown on x-axis
plt.savefig("allocstall_movable_uni_plot.png")
